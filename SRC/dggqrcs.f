...
*  ===========
*
*       SUBROUTINE DGGQRCS( JOBU1, JOBU2, JOBX,
...
*>          = 'Y':  Advises DGGQRCS to pre-process A;
*>          = 'N':  advises DGGQRCS not to pre-process A;
...
*>          = 'Y':  Advises DGGQRCS to pre-process B;
*>          = 'N':  advises DGGQRCS not to pre-process B;
*>          = '?':  pre-processing decision left to algorithm.
*>
*>          On exit, the variable will be set to 'Y' if the matrix
*>          was pre-processed, 'N' otherwise.
...
*>          = 'Y':  Advises DGGQRCS to apply column pre-processing;
*>          = 'N':  advises DGGQRCS not to apply column pre-processing;
...
*  =====================
*>
*>  DGGQRCS can pre-process the input matrices A and B. Pre-processing
...
*>  passed as hint. DGGQRCS may ignore these _hints_, in particular for
...
*>
*>  DGGQRCS has a dedicated pre-processing step for matrix pairs with
...
*>  does not care, '?' should be passed as hint. DGGQRCS may ignore
...
*>
*  =====================================================================
      RECURSIVE SUBROUTINE DGGQRCS( JOBU1, JOBU2, JOBX,
     $                              HINTPREPA, HINTPREPB, HINTPREPCOLS,
     $                              M, N, P, RANK,
     $                              SWAPPED,
     $                              A, LDA, B, LDB,
...
*
*     .. Parameters ..
      DOUBLE PRECISION   ONE, ZERO, REALONE, REALZERO
...
*     .. External Subroutines ..
      EXTERNAL           DGEMM, DGEQP3, DGETRP, DLACPY, DLAPMT, DLASCL,
     $                   DLASET, DORGQR, DORCSD2BY1, DORMLQ, DORMQR,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( PREPROCESSCOLS ) THEN
         CALL DGEQP3( N, M + P, WORK( IMAT ), LDG, IWORK, WORK,
     $                WORK, -1, INFO )
...
*
      IF( PREPROCESSA ) THEN
         CALL DGEQP3( M, N, A, LDA, IWORK, WORK( ITAUA ),
     $                WORK, -1, INFO )
...
*
      IF( PREPROCESSB ) THEN
         CALL DGEQP3( P, N, B, LDB, IWORK, WORK( ITAUB ),
     $                WORK, -1, INFO )
...
*
      IF( PREPROCESSCOLS ) THEN
         CALL DGEQRF( ROWSA + ROWSB, RANKMAXG, WORK, LDG, WORK,
     $                WORK, -1, INFO )
         LWKMIN = MAX( LWKMIN, INT( WORK( 1 ) ) + ISCRATCH )
         LWKOPT = MAX( LWKOPT, INT( WORK( 1 ) ) + ISCRATCH )
      ELSE
         CALL DGEQP3( ROWSA + ROWSB, N, WORK( IMAT ), LDG, IWORK,
     $             WORK( ITAUGR ), WORK, -1, INFO )
         LWKMIN = MAX( LWKMIN, 3 * N + 1        + ISCRATCH )
         LWKOPT = MAX( LWKOPT, INT( WORK( 1 ) ) + ISCRATCH )
      ENDIF
*
      CALL DORGQR( ROWSA + ROWSB, RANKMAXG, RANKMAXG, WORK( IMAT ), LDG,
     $             WORK( ITAUGR ), WORK, -1, INFO )
      LWKMIN = MAX( LWKMIN, RANKMAXG         + ISCRATCH )
      LWKOPT = MAX( LWKOPT, INT( WORK( 1 ) ) + ISCRATCH )
*
      CALL DORCSD2BY1( JOBU1, JOBU2, JOBX,
...
*
      IF( PREPROCESSCOLS .AND. WANTX ) THEN
         I = ITAUGR - ITAUGL
         CALL DORMQR( 'R', 'T', I, N, I, WORK, LDG, WORK, X, LDX,
     $                WORK, -1, INFO )
...
*
      IF( PREPROCESSA .AND. WANTU1 ) THEN
         CALL DORMQR( 'L', 'N', M, M, RANKMAXA, A, LDA,
     $                WORK, U1, LDU1,
     $                WORK, -1, INFO )
...
*
      IF( PREPROCESSB .AND. WANTU2 ) THEN
         CALL DORMQR( 'L', 'N', P, P, RANKMAXB, B, LDB,
     $                WORK, U2, LDU2,
     $                WORK, -1, INFO )
         LWKMIN = MAX( LWKMIN, MAX( 1, N )      + IMAT )
         LWKOPT = MAX( LWKOPT, INT( WORK( 1 ) ) + IMAT )
...
*
      ...
      ENDIF
*
      CALL DLASCL( 'G', -1, -1, REALONE, W, M, N, A, LDA, INFO )
      IF ( INFO.NE.0 ) THEN
         RETURN
      ENDIF
*
...
*
      IF( PREPROCESSCOLS ) THEN
*        assert!( m + p <= n );
         CALL DGETRP( M, N, A, LDA, WORK( IMAT ), LDG, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
         ENDIF
         CALL DGETRP( P, N, B, LDB, WORK( IMAT + M * LDG ), LDG, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
         ENDIF
*
         IWORK( 1:M+P ) = 0
         CALL DGEQP3( N, M + P, WORK( IMAT ), LDG,
     $                IWORK, WORK( ITAUGL ),
...
*        Multiply orthogonal factor with A
         CALL DORMQR( 'R', 'N', M, N, RANK, WORK( IMAT ), LDG,
     $                WORK( ITAUGL ), A, LDA,
     $                WORK( ITAUGR ), LWORK - ITAUGR + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
...
*        Multiply orthogonal factor with B
         CALL DORMQR( 'R', 'N', P, N, RANK, WORK( IMAT ), LDG,
     $                WORK( ITAUGL ), B, LDB,
     $                WORK( ITAUGR ), LWORK - ITAUGR + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
...
*        re-used by G
         IF( WANTX ) THEN
            CALL DLACPY( 'L', M + P, COLS, WORK( IMAT ), LDG,
     $                   X( 1, COLS + 1 ), LDX )
         ENDIF
      ENDIF
*
...
*
      IF( PREPROCESSA ) THEN
         IWORK( 1:COLS ) = 0
         CALL DGEQP3( M, COLS, A, LDA, IWORK, WORK( ITAUA ),
     $                WORK( ITAUGR ), LWORK - ITAUGR + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
         END IF
...
*        Copy full rank part into G
         CALL DLASET( 'L', ROWSA, COLS, ZERO, ZERO,
     $                WORK( IG11 ), LDG )
         CALL DLACPY( 'U', ROWSA, COLS, A, LDA, WORK( IG11 ), LDG )
         CALL DLAPMT( .FALSE., ROWSA, COLS, WORK( IG11 ), LDG, IWORK )
*        Initialize U1 although xORCSDB2BY1 will partially overwrite this
         IF( WANTU1 ) THEN
             CALL DLASET( 'A', M, M, ZERO, ONE, U1, LDU1 )
         ENDIF
      ELSE
         CALL DLACPY( 'A', M, COLS, A, LDA, WORK( IG11 ), LDG )
      END IF
*
      IF( PREPROCESSB ) THEN
         IWORK( 1:COLS ) = 0
         CALL DGEQP3( P, COLS, B, LDB, IWORK, WORK( ITAUB ),
     $                WORK( ITAUGR ), LWORK - ITAUGR + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
         END IF
*        Determine rank
         ...
         END DO
*        Copy full rank part into G
         CALL DLASET( 'L', ROWSB, COLS, ZERO, ZERO,
     $                WORK( IG21 ), LDG )
         CALL DLACPY( 'U', ROWSB, COLS, B, LDB, WORK( IG21 ), LDG )
         CALL DLAPMT( .FALSE., ROWSB, COLS, WORK( IG21 ), LDG, IWORK )
*        Initialize U2 although xORCSDB2BY1 will partially overwrite this
         IF( WANTU2 ) THEN
            CALL DLASET( 'A', P, P, ZERO, ONE, U2, LDU2 )
         ENDIF
      ELSE
         CALL DLACPY( 'A', P, COLS, B, LDB, WORK( IG21 ), LDG )
      END IF
*
*
...
*        assert!( COLS < N );
         CALL DGEQRF( ROWSA + ROWSB, COLS, WORK( IG11 ), LDG,
     $                WORK( ITAUGR ),
...
*        Compute the QR factorization with column pivoting GΠ = Q1 R1
         IWORK( 1:N ) = 0
         CALL DGEQP3( ROWSA + ROWSB, COLS, WORK( IG11 ), LDG, IWORK,
     $                WORK( ITAUGR ),
     $                WORK( ISCRATCH ), LWORK - ISCRATCH + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
...
*
      IF( WANTX ) THEN
         IF( RANK.LE.M ) THEN
            CALL DLACPY( 'U', RANK, COLS, WORK( IG11 ), LDG, A, LDA )
         ELSE
            CALL DLACPY( 'U', M, COLS, WORK( IG11 ), LDG, A, LDA )
            CALL DLACPY( 'U', RANK - M, COLS - M, WORK( IG22 ), LDG,
     $                   B, LDB )
         END IF
      END IF
*     DEBUG
      IF( PREPROCESSCOLS ) THEN
          CALL DLASET( 'U', N, COLS, NAN, NAN, WORK( IG11 ), LDG )
      ELSE
          CALL DLASET( 'U', RANK, COLS, NAN, NAN, WORK( IG11 ), LDG )
      ENDIF
*
*     Explicitly form Q1 so that we can compute the CS decomposition
*
      CALL DORGQR( ROWSA + ROWSB, RANK, RANK, WORK( IG11 ), LDG,
     $             WORK( ITAUGR ),
     $             WORK( ISCRATCH ), LWORK - ISCRATCH + 1, INFO )
      IF ( INFO.NE.0 ) THEN
         RETURN
...
*
      CALL DORCSD2BY1( JOBU1, JOBU2, JOBX, ROWSA + ROWSB, ROWSA, RANK,
     $                 WORK( IG11 ), LDG, WORK( IG21 ), LDG,
     $                 BETA( K1P - K1 + 1 ),
     $                 U1, LDU1, U2, LDU2, X, LDX,
...
*        Move upper half of elementary reflectors of LQ back
         IF( PREPROCESSCOLS ) THEN
            CALL DLACPY( 'L', M + P, RANK, X( 1, COLS + 1 ), LDX,
     $                   WORK( IG11 ), LDG )
*           Debug
            CALL DLASET( 'A', M + P, COLS - RANK, NAN, NAN,
     $                   X( 1, COLS + 1 ), LDX )
         ENDIF
*
         IF ( RANK.LE.M ) THEN
           IF( COLS.GT.RANK ) THEN
               CALL DGEMM( 'N', 'N', RANK, COLS - RANK, RANK,
     $                     ONE, X, LDX, A( 1, RANK + 1 ), LDA,
     $                     ZERO, X( 1, RANK + 1 ), LDX )
            END IF
            CALL DTRMM( 'R', 'U', 'N', 'N', RANK, RANK, ONE,
     $                  A, LDA, X, LDX )
         ELSE
            CALL DLACPY( 'U', M, COLS, A, LDA, WORK( IG11 ), LDG )
            CALL DLACPY( 'U', RANK - M, COLS - M, B, LDB,
     $                   WORK( IG22 ), LDG )
            IF( COLS.GT.RANK ) THEN
               CALL DGEMM( 'N', 'N', RANK, COLS - RANK, RANK,
     $                     ONE, X, LDX,
...
*
         IF( PREPROCESSCOLS ) THEN
            CALL DLASET( 'G', RANK, N - RANK, ZERO, ZERO,
     $                   X( 1, RANK + 1 ), LDX )
            CALL DORMQR( 'R', 'T', RANK, N, RANK, WORK( IMAT ), LDG,
     $                   WORK( ITAUGL ), X, LDX,
     $                   WORK( ITAUGR ), LWORK - ITAUGR + 1, INFO )
         ELSE
*           Revert column permutation Π by permuting the columns of X
            CALL DLAPMT( .FALSE., RANK, COLS, X, LDX, IWORK )
         ENDIF
      ENDIF
*     DEBUG
      WORK( IMAT:LWORK ) = NAN
...
*
      IF( PREPROCESSA .AND. WANTU1 ) THEN
         CALL DORMQR( 'L', 'N', M, M, ROWSA, A, LDA,
     $                WORK( ITAUA ), U1, LDU1,
...
*
      IF( PREPROCESSB .AND. WANTU2 ) THEN
         CALL DORMQR( 'L', 'N', P, P, ROWSB, B, LDB,
     $                WORK( ITAUB ), U2, LDU2,
     $                WORK( IMAT ), LWORK - IMAT + 1, INFO )
         IF( INFO.NE.0 ) THEN
            RETURN
...
*        overwriting IWORK.
         IF( ROWSB.LT.P ) THEN
            ...
            ELSE
               ...
               ENDDO
            ENDIF
            CALL DLAPMT( .FALSE., P, P, U2, LDU2, IWORK )
         ENDIF
      ENDIF
*
*     Debug
